...
#include "m_pd.h"
#include "s_stuff.h"  /* for sys_addpollfn() sys_rmpollfm() */
...
/* 
 * No longer needed since clock_delay() has been swapped for sys_addpollfn() 
 */
...
/*
 * these are in s_stuff.h, which is now included because of sys_*pollfn()
 */
...
static int netclient_poll(t_netclient *x, int fd)
{
    if(x->x_connectstate)
		netclient_rcv(x);	/* if we're connected, try to read */
...
static void *netclient_child_connect(void *w)
{
    ...
    x->x_fd = sockfd;
	 sys_addpollfn(x->x_fd, (t_fdpollfn)netclient_poll, x);
...
/*     x->x_poll = clock_new(x, (t_method)netclient_poll); */
    ...
    if(pthread_attr_setdetachstate(&x->x_threadattr, PTHREAD_CREATE_DETACHED) < 0)
       post("netclient: warning: could not prepare child thread" );
	/* start polling the input */
/* 	 clock_delay(x->x_poll, 0); */
...
void netclient_setup(void)
{
	...
	class_addmethod(netclient_class, (t_method)netclient_rcv, gensym("rcv"), 0);
    	/* removed to be compatible with 0.37 */ 
