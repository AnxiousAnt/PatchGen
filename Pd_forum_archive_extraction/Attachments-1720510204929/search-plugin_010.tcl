...
proc ::dialog_helpbrowser2::create_dialog {mytoplevel} {
    ...
    ttk::combobox $mytoplevel.f.searchtextentry \
        -textvar ::dialog_helpbrowser2::searchtext \
...
proc ::dialog_helpbrowser2::find_doc_files { basedir recursive } {
    # Todo: now that we're using xapian there's no reason to
    # build a list of files-- a list of directories should be
    # enough to build the index.  (Though we should still probably
    # filter out directories that have a parent directory in the
    # list...
...
proc ::dialog_helpbrowser2::search { } {
    ...
    $widget insert 1.end " Building an index for all subsequent searches..."
    $widget configure -state disabled
...
proc ::dialog_helpbrowser2::do_search { } {
    ...
    # todo: remove the searchfile_sort for indexing
...
proc ::dialog_helpbrowser2::results_epilog {widget} { 
    ...
    $widget configure -state normal
    if {[lsearch [$widget window names] .helpbrowser2.navtext.pbar] != -1} {
        $widget delete .helpbrowser2.navtext.pbar end
    }
...
proc ::dialog_helpbrowser2::db_error {exception} {
...
proc ::dialog_helpbrowser2::build_index {} {
    variable database
    variable filelist
    variable progress
    variable i
	...
	set basedir [lindex $filelist $i 2]
        ...
        if {[catch {
            ...
            set doc_contents "docname $docname\ndocfile $docfile\nbasedir $basedir\n$file_contents"

            xapian::Document doc

            doc set_data $doc_contents
	...
	# loops. Hm...
	    ...
	    if { $cancelled == 0 } {
	        ...
	        after idle ::dialog_helpbrowser2::build_index
	    } else {
        ...
        } else { ::dialog_helpbrowser2::build_index }
    } else {
...
proc ::dialog_helpbrowser2::do_query {} {
    variable database
    variable searchtext

    if {[catch {
        xapian::Database database doc_index
        ...
        db_error $exception
        results_epilog .helpbrowser2.navtext
        return
    }
    if {[catch {
...
#        pdtk_post "[document get_data]\n\n\n"
        regexp {docname (.*?)\n} $data - docname
        regexp {basedir (.*?)\n} $data - basedir
        printresult $docname $basedir $data .helpbrowser2.resultstext 1
    }
    results_epilog .helpbrowser2.navtext
}

# this probably isn't needed with Xapian backend
...
proc ::pdmanual_description {filename} {
    ...
    switch -exact $filename {
        ...
        x5.htm { set desc "Pd Documentation Chatper 5: Current Status of the Software" }
    }
    return $desc
}
