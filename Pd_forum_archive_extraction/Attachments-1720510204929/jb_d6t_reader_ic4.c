...
int main (int argc, char **argv)
{
  ...
  do
  {
    ...
    // read second sensor, a D6T44L
    sensor = 1;
    bufsize = D6T44L2_SENSOR_PACKET_SIZE; // sensor 1 is D6T44l2
    ...
    do
    {
      ...
      // clear the inbuf first
      for ( i = 0; i < D6T44L2_SENSOR_PACKET_SIZE; ++i) inbuf[i] = 0;
      ...
      if (read (i2c_file, inbuf, D6T44L2_SENSOR_PACKET_SIZE) != D6T44L2_SENSOR_PACKET_SIZE)
      {
        printf("Read failed (%d)\n", errno);
        buffer = strerror(errno);
        printf(">> %s <<\n", buffer);
      }
    } while (!D6T_checkPEC (inbuf, D6T44L2_SENSOR_PACKET_SIZE-1));
    // print to console
    for (i = 0; i < D6T44L2_SENSOR_PACKET_SIZE-1; i += 2)
    {
      printf("%d ", inbuf[i]+(inbuf[i+1]<<8));
    }
    printf("<0x%02X>\n", inbuf[bufsize-1]);
    // send to Pd
    j = sprintf (netbuf, D6T44L2_PD_SELECTOR);
    for (i = 0; i < D6T44L2_SENSOR_PACKET_SIZE-1; i += 2)
    {
      j += sprintf(&netbuf[j], " %d", inbuf[i] + (inbuf[i+1]<<8));
    }
