...
namespace eval ::dialog_search:: {
    ...
    variable count {}
    variable genres [list "All documents" \
			"Object Help Patches" \
			"All About Pd" \
			"Tutorials" \
			"Manual" \
...
proc ::dialog_search::font_size {text direction} {
    ...
    if {$update} {
	    ...
	    set val [$text tag cget $tag -font]
	    if {[string is digit -strict [lindex $val 1]]} {
	        $text tag configure $tag -font "Helvetica [expr {max([lindex $val 1]+$offset,$min_fontsize)}]"
	    }
        }
    }
}
...
proc ::dialog_search::create_dialog {mytoplevel} {
    ...
    variable genres
    toplevel $mytoplevel
    wm title $mytoplevel "Search"
    wm geometry $mytoplevel 500x500+0+0
    ...
    ttk::button $mytoplevel.searchbutton -text "Search" -takefocus 0 \
    ...
    ttk::checkbutton $mytoplevel.f.matchall_check -text "Match all terms" \
	-variable ::dialog_search::matchall
    ttk::checkbutton $mytoplevel.f.matchwords_check -text "Match whole words only" \
	-variable ::dialog_search::matchwords
    ttk::label $mytoplevel.advancedlabel -text "Advanced" -foreground "#0000ff" \
	-anchor center
    ...
    grid $mytoplevel.advancedlabel -column 3 -columnspan 2 -row 1 -sticky we 
...
dialog_search::create_dialog .search
