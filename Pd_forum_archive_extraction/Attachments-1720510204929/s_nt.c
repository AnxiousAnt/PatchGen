...
/* modified 05/17/02 by Chris Campbell to fix ASIO device selection bug*/
    ...
    int nchoutdev, int *choutdev, int rate) /* IOhannes */
{
    ...
    {
	/*RM MOD CCAMPBELL 051702
	pa_open_audio(inchans, outchans, rate, sys_soundin, sys_soundout,
    	   256, nt_advance_samples/256,
	  (naudiooutdev > 0 ? -1 : audiooutdev[0]-1));
	  END RM MOD CCAMPBELL 051702*/
	
	//BEG MOD CCAMPBELL 051702
	char msg[40];
	int iOutDev;
	
	//If invalid audio outdev, then pass -1 to trigger portaudio 
	//to query the system for the default device.
	//otherwise, use the first audioout device specified.
	iOutDev =  (naudiooutdev < 0 ? -1 : audiooutdev[0]);
	sprintf(msg, "Initializing PortAudio with device: %d", iOutDev);
	post(msg);
	pa_open_audio(inchans, outchans, rate, sys_soundin, sys_soundout,
		      256, nt_advance_samples/256, iOutDev);
	//END MOD CCAMPBELL 
    }
    else mmio_open_audio();
}
