...
static t_symbol *sspin_sym;

static t_class *sspin_class;

typedef struct _sspin
{
    t_object x_obj;
    t_float x_channel;
    t_outlet *x_outlet1;
...
static void *sspin_new(t_floatarg f)
{
    t_sspin *x = (t_sspin *)pd_new(sspin_class);
    x->x_channel = f;
    x->x_outlet1 = outlet_new(&x->x_obj, &s_float);
    if (f == 0) x->x_outlet2 = outlet_new(&x->x_obj, &s_float);
    pd_bind(&x->x_obj.ob_pd, sspin_sym);
    return (x);
}

static void sspin_list(t_sspin *x, t_symbol *s, int argc, t_atom *argv)
{
    t_float value = atom_getfloatarg(0, argc, argv);
    t_float channel = atom_getfloatarg(1, argc, argv);
    if (x->x_channel != 0)
...
static void sspin_free(t_sspin *x)
{
    pd_unbind(&x->x_obj.ob_pd, sspin_sym);
}

static void sspin_setup(void)
{
    sspin_class = class_new(gensym("sspin"), (t_newmethod)sspin_new,
        (t_method)sspin_free, sizeof(t_sspin), CLASS_NOINLET, A_DEFFLOAT, 0);
    class_addlist(sspin_class, sspin_list);
    class_sethelpsymbol(sspin_class, gensym("midi"));
    sspin_sym = gensym("#sspin");
}

void inmidi_ssp(int portno, int channel, int value)
{
    if (sspin_sym->s_thing)
    {
        t_atom at[2];
        SETFLOAT(at, value);
        SETFLOAT(at+1, (channel + (portno << 4) + 1));
        pd_list(sspin_sym->s_thing, &s_list, 2, at);
    }
}
